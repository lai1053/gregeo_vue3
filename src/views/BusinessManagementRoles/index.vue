          ...
          <div>
            ...
            <div class="titleAdd">
              ...
              <span @click="titleAddClick(null, null)">
                <a-tooltip title="新增角色"> <IconFont type="iconAddto" /></a-tooltip>
              </span>
            </div>
            <a-tree
              ...
              <template #title="{ title, key }">
                <span v-if="title.indexOf(searchValue) > -1">
                  ...
                  <span class="featuresHover">
                    ...
                    <a><span @click="titleModifyClick(title, key)"> <IconFont type="iconedit" /></span></a>
                  </span>
                </span>
...
import { getDepartment, createRole, modifyRole, getUserList } from '@/api/businessManagementRoles'
      ...
      this.proFormData.formConfig = [
        {
          ...
          key: 'name',
          component: 'MetaInput',
          rules: [{ required: true, message: '角色需要输入', type: 'string', trigger: 'blur' }],
          span: 24,
          labelCol: {
            span: 12,
          },
        ...
        },
        {
          span: 24,
          label: '等级',
          rules: [{ required: true, message: '等级需要输入', type: 'any', trigger: 'blur' }],
          labelCol: {
            span: 12,
          },
          wrapperCol: {
    ...
    async sendCode(data) {
      ...
      switch (modalTitle) {
        case '新增':
          ...
          res = await createRole(createRoleParameters)
          ...
          break
        case '修改':
          const modifyRoleParameters = {
            id: data.id,
            level: data.level,
            name: data.name,
            description: data.description,
          }
          res = await modifyRole(modifyRoleParameters)
          if (res.code === 0) {
    ...
    async menuClick(key, data) {
      ...
      switch (key) {
        case 'add':
          ...
          role.result.forEach(i => {
            i.value = i.id
          })
          // 把角色信息和部门信息放入配置当中
          departmentList.forEach(i => {
            if (i.key === 'role') {
              i.props.option = role.result
            }
            if (i.key === 'department') {
              i.props.option = [
                {
